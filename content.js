var qrcode_generator = qrcode_generator || {};
...
(function(window, undefined) {
  ...
  window.qrcode_generator.jQuery = jQuery;
})(window);

//QRcode jQuery plugin
(function($) {
  'use strict';
...
(function(QG) {
  ...
  function generateQRcode(text) {
    text = utf16to8(text);
    var html = '<div id="qrcode-wrapper">' +
      '<div class="qrcode-title">二维码生成<span class="qrcode-close" title="关闭">&times</span></div>' +
      '<div class="qrcode-content"></div>' +
      '<div class="qrcode-footer">使用 ' +
      '<a href="http://www.wochacha.com" target="_blank" title="点击下载我查查">我查查</a>' +
      ' 扫一扫</div>' +
      '</div>';
    ...
    if ($('#qrcode-wrapper').length === 0) {
      ...
      //如果已有二维码则删除
      $('#qrcode-wrapper').children('.qrcode-content').empty();
    }

    $('#qrcode-wrapper').children('.qrcode-content').qrcode({
      fill: '#333',
      size: 160,
      text: text
    });
  ...
  $('body').on('click', '.qrcode-close', function(event) {
    $(this).closest('#qrcode-wrapper').fadeOut('slow', function() {
      $(this).remove();
    });
  });
